...
BEGIN_MESSAGE_MAP(CPropertiesWnd, CDockablePane)
	...
	ON_WM_SETFOCUS()
	ON_COMMAND(ID_PROPERTIES_TESTBTN, OnBtnClick)
	ON_UPDATE_COMMAND_UI(ID_PROPERTIES_TESTBTN, OnUpdateButton)
	ON_COMMAND_RANGE(ID_PROPERTIES_CHECKBOX1, ID_PROPERTIES_CHECKBOX6, OnChecked)
	// ON_WM_SETTINGCHANGE()
	ON_WM_DESTROY()
	// ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()
...
int CPropertiesWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//m_wndObjectCombo.AddString(_T("테스트 애플리케이션"));
	...
	CRect rectClient;
	GetClientRect(rectClient);
	LONG left = rectClient.left;
	LONG right = rectClient.right;

	m_testBtn = new CButton();
	CString caption = _T("버튼");
	m_testBtn->Create(caption, WS_CHILD | WS_VISIBLE, CRect(left, 0, right, 30), this, ID_PROPERTIES_TESTBTN);

	checkBox = new CButton[nOPT_LTG];

	for (int i = 0; i < nOPT_LTG; i++) {
		checkBox[i].Create(checkBoxLabel[i], WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, CRect(left, 30 + height * i, right, 30 + height * (i + 1)), this, ID_PROPERTIES_CHECKBOX1 + i);
		checkBox[i].SetCheck(TRUE);
	}
...
void CPropertiesWnd::OnBtnClick() {
	...
	AfxGetMainWnd()->SendMessage(UWM_CUSTOM1);
}
...
void CPropertiesWnd::OnChecked(UINT nID) {
	if (!cOpenGLProjectView)
		cOpenGLProjectView = (COpenGLProjectView*)((CMainFrame*)AfxGetMainWnd())->GetActiveView();
	int index = nID - ID_PROPERTIES_CHECKBOX1;
	// TRUE : 체크됨
	// FALSE : 해제됨
	if (checkBox[index].GetCheck()) {
		cOpenGLProjectView->SendMessage(UWM_CHECKED, TRUE, nID);
	}
	else {
		cOpenGLProjectView->SendMessage(UWM_CHECKED, FALSE, nID);
	}
}